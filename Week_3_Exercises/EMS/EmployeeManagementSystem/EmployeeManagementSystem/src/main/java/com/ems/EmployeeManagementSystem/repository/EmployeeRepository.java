...
import com.ems.EmployeeManagementSystem.projection.EmployeeNameAndDepartmentProjection;
import com.ems.EmployeeManagementSystem.projection.EmployeeNameAndSalary;
...
public interface EmployeeRepository extends JpaRepository<Employee, Long> {
    ...
    @Query("SELECT e.firstName AS firstName, e.lastName AS lastName, d.name AS departmentName FROM Employee e JOIN e.department d")
    List<EmployeeNameAndDepartmentProjection> findEmployeeNameAndDepartment();

    @Query("SELECT new com.example.employeemanagement.projection.EmployeeNameAndSalary(e.firstName, e.lastName, e.salary) FROM Employee e")
    List<EmployeeNameAndSalary> findEmployeeNameAndSalary();
}
